...
// Load .
func (h HTTP) Load(cfg interfaces.Config) error {
...
// Save .
func (h HTTP) Save(cfg interfaces.Config) error {

	data, err := cfg.Marshaler().Marshal(cfg)
	if err != nil {
		return err
	}

	resp, err := h.httpNewRequest("PUT", h.query(cfg.Path()), bytes.NewReader(data))
	if err != nil {
		return err
	}
	defer resp.Body.Close()
